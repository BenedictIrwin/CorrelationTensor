...
## Loop over tags
for tag in ["min_f1","min_f2","min_f12","4AA_f1","4AA_f2","4AA_f12","5AA_f1","5AA_f2","5AA_f12","6AA_f1","6AA_f2","6AA_f12"]:
  ...
  if(True):
    ...
    if(tag=="min_f12"):
      for i,j in zip(tag_to_dist.keys(),tag_to_dist.values()):
        plt.plot([j+40, j+40], [-65, 6], 'k--', lw=1, color =fc)
        shif = 0.0
        if(i=="CC"): shif = 0.4
        if(i=="OH3"): shif = -0.4
        plt.text(j+40-0.8+shif,6.6,i, fontsize = 18, rotation = 90, ha = "left", va = "bottom", color = fc)

    if(tag == "min_f1"): minilabel = "Min. AB" 
    if(tag == "min_f2"): minilabel = "Min. DE" 
    if(tag == "min_f12"): minilabel = "Min. AB+DE" 
    if(tag == "4AA_f1"): minilabel = "4$\AA$ AB" 
    if(tag == "4AA_f2"): minilabel = "4$\AA$ DE" 
    if(tag == "4AA_f12"): minilabel = "4$\AA$ AB+DE" 
    if(tag == "5AA_f1"): minilabel = "5$\AA$ AB" 
    if(tag == "5AA_f2"): minilabel = "5$\AA$ DE" 
    if(tag == "5AA_f12"): minilabel = "5$\AA$ AB+DE" 
    if(tag == "6AA_f1"): minilabel = "6$\AA$ AB" 
    if(tag == "6AA_f2"): minilabel = "6$\AA$ DE" 
    if(tag == "6AA_f12"): minilabel = "6$\AA$ AB+DE" 
    plt.text(95,np.mean(highs)+2,minilabel,fontsize=18, color = fc)
    ...
    if(tag=="min_f12"):
      ...
      plt.plot(new_x,close_spline(new_x),'-',color='black',linewidth=3, label="new pore")
    else:
      plt.plot(new_x,open_spline(new_x),'-',color='black',linewidth=1)
      plt.plot(new_x,close_spline(new_x),'-',color='black',linewidth=3)
...
plt.show()
fig.savefig('{}_LARGE.png'.format("ALL_Stacked"), dpi=300)
