...
## Loop over tags
for tag in ["min_f1","min_f2","min_f12","6AA_f1","6AA_f2","6AA_f12"]:
  ...
  if(True):
    ...
    if(tag=="min_f12"):
      for i,j in zip(tag_to_dist.keys(),tag_to_dist.values()):
        ...
        if(i=="CC"): shif = 0.4
        if(i=="OH3"): shif = -0.4
        plt.text(j+40-0.8+shif,7.55,i, fontsize = 18, rotation = 90, ha = "left", va = "bottom", color = fc)
    ...
    plt.text(90,np.mean(highs)+2,minilabel,fontsize=18, color = fc)
